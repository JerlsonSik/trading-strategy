#This is for getting all the data down from ETHUSDT
...
def init_mod():
    ...
    binance_api_key = "t6dxBytsovc88DlFKHNb6sFmhgK6Pud7mMuNJ16JSLLjr5o8QkJV02FB8qd3iF8f"
    binance_api_secret = "St8QBQ3kB8O4Ru3D9yO1aN0mNoeeCRbcyjYEORcF2IimPHyevWpDoZRqQjWPDPkQ"
...
def concat_binance_data(symbol_list, output_filename):
    ...
    new_columns = [item + '_' + 'ETHUSDT' for item in columns_of_interest]
    # new_columns.insert(0, 'timestamp')
...
if __name__ == '__main__':
    ...
    to_date = '2023-04-01 00:00:00'
    symbol_list = ['ETHUSDT']

    for num, symbol in enumerate(symbol_list):
        output_filename = '%s-binance-datas.csv' % (symbol)
        # print('-' * 60)
        # print('Downloading data from %s to %s for %s' % (from_date, to_date, symbol))
        # print('-' * 60)
        download_data_from_binance(symbol, from_date, to_date, output_filename, step=100, pause=0, simulate=False)
